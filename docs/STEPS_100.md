...
20. 🔧 Branch `cursor/apex-clobber-runbook-ff6e` on councilof-ai
...
24. ⏳ Assert apex ≥20KB + CouncilLobby chunk
25. ⏳ Assert `/lobby` `/scorecard` `/honesty` `/gspc-verify/` 200
26. ⏳ Assert `/` still fat ~2 min later (clobber window)
27. ⏳ Drift-guard green
28. ⏳ Fix sitemap URLs that REAL-404
29. ⏳ Align `/api/chat` public_count with totals
30. ⏳ LivingBoard on home
31. ⏳ Moody’s scorecard on apex `/scorecard`
32. ⏳ Honesty Elo disclosure page (internal only)
33. ⏳ Live ledger route
34. ⏳ Verify free route
...
- 👤 Re-run deploy workflow; confirm apex fat
