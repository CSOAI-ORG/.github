...
112. ✅ Patch bundle `docs/patches/councilof-ai-openrouter-agui/` (historical; superseded by one-door)
113. ✅ `scripts/e2e-integration-stack.mjs` in .github (live smoke)
114. ✅ One-door policy — `/ag-ui` → `/?lobby=home` (Council OS; `one-door-guard.yml`)
115. ✅ Merge csoai-static-deploy2 **PR #37** (static ag-ui reference host)
116. 👤 Set `AGUI_WIRE_URL` on Cloudflare Pages
117. ✅ Fat apex restored 2026-08-24 (~214KB) after App.tsx conflict fix (PR #420/#421)
118. ✅ Static ag-ui has `council-chat-ask` bridge on `csoai-site.pages.dev`
119. ⏳ `e2e-integration-stack.mjs` green after clobber settles
...
- 👤 `HF_TOKEN` → `bash scripts/upload-hf-patches.sh` (14 axis cards ready)
- See [`STEPS_200.md`](STEPS_200.md) for moves 121–220
