# Restore CF_API_TOKEN for MCP worker redeploy (owner action)

**Why:** Official MCP registry is **1.0.3** (`board (14 of 14)`), but live Workers still answer `initialize.serverInfo.version = **1.0.0**`:

- `https://councilof.ai/mcp`
- `https://csoai-gspc-mcp.nicholastempleman.workers.dev/mcp`

Source on `csoai-static-deploy2` already says **1.0.3** (`workers/csoai-gspc-mcp/src/index.js` + `package.json`). Deploy workflow:
...
1. Cloudflare dashboard → My Profile → API Tokens → create token with **Edit Cloudflare Workers** (account scoped to CSOAI).
