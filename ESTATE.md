...
**Cloud agents and automations.** Start them against [CSOAI-ORG/councilof-ai](https://github.com/CSOAI-ORG/councilof-ai), not this profile repo. A multi-repo environment may include this repo for community-health files; the working tree for site and Council OS is `councilof-ai` on `master`. Official success is `.github/workflows/deploy.yml` **build-deploy**. Ignore Vercel reds. Published index is 313 verifying cards — do not restore exact-150.
