...
const AXES: AxisScore[] = [
  ...
  // of the per-model rates (recomputable from the rows below).
  {
    ...
    note: "The jail floor is no longer empty: measured 2026-08-17/18 on a 71-cell gold bank. " +
      ...
      "accuracy 0.5412. Our own council-inhouse-ft fine-tune detected ZERO escapes (tp=0) — published, " +
...
export const onRequestGet: PagesFunction = async (context) => {
  ...
  const body = {
    ...
    limitations: [
      "4 of the 13 canonical axes show a statistically separated leader (McNemar p<0.05 on discordant items, or Wilson-bound non-overlap for swarm): governance, care, swarm, affect. 9 are statistical ties — a point-estimate lead is not a measured advantage.",
